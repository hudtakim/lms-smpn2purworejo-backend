...
func GetTeacherListToPlot(w http.ResponseWriter, r *http.Request) {
	rows, err := config.Pool.Query(context.Background(),
		"SELECT id, username, full_name, role, is_active, gender, religion FROM users WHERE role = 'teacher' AND is_active = true ORDER BY full_name ASC")
	if err != nil {
		serverError(w, r, err, "Internal server error")
		return
	}
