...
func GetQuizScores(w http.ResponseWriter, r *http.Request) {
	...
	var kkmStr string
	config.Pool.QueryRow(context.Background(),
		"SELECT setting_value FROM app_settings WHERE setting_key = 'default_kkm'").Scan(&kkmStr)
	if v, err := strconv.ParseFloat(kkmStr, 64); err == nil {
		globalKKM = v
	}
